...
function updateState({ enabled, darkMode }) {
  console.log("updateState", enabled);
  ...
  sendToAllTabs(enabled, darkMode);
  return true;
}
...
function fetchAndSendToAllTabs() {
  chrome.storage.sync.get(["enabled", "darkMode"], (result) => {
    ...
    sendToAllTabs(result.enabled, result.darkMode);
  });
}

function sendToAllTabs(enabled, darkMode) {
  ACTIVE_TABS.forEach((tab) => {
    chrome.tabs.sendMessage(tab, {
      type: "state_sync",
      value: { enabled, darkMode },
    });
  });
}
